"""Lightweight beam search v0 for Orbit Wars."""
...
try:
    from ..strategy.geometry import angle_to, avoidance_angle, distance, fleet_speed
    from ..strategy.targeting import PlanetView, pick_best_target, score_target
    from ..strategy.threat import FleetView, incoming_threat
except (ImportError, KeyError):
    from strategy.geometry import angle_to, avoidance_angle, distance, fleet_speed  # type: ignore[no-redef]
    from strategy.targeting import PlanetView, pick_best_target, score_target  # type: ignore[no-redef]
    ...
    for mine in sorted(my_planets, key=lambda planet: planet.id):
        ...
        decisions.append(
            Action(
                ...
                angle=float(
                    avoidance_angle(mine.x, mine.y, target.x, target.y, margin=1.0)
                ),
            )
        )
    return decisions
...
def _candidate_actions_for_planet(
    ...
    friendlies = [
        planet
        for planet in state.planets
        if planet.owner == player and planet.id != source.id
    ]

    ranked: list[Action] = []

    def add_ranked(
        planets: list[ProjectedPlanet], limit: int, friendly: bool = False
    ) -> None:
        if not planets:
            return
        scored = sorted(
        ...
        for target in scored:
            ...
            ranked.append(
                Action(
                    ...
                    angle=float(
                        avoidance_angle(
                            source.x, source.y, target.x, target.y, margin=1.0
                        )
                    ),
                )
            )
...
def _apply_action(
    state: SearchState, action: Action, root_depth: bool = False
) -> SearchState:
...
def _score_state(state: SearchState, player: int) -> float:
    ...
    for planet in state.planets:
        ...
        if planet.owner == player:
            if targets:
                roi = max(
                    score_target(planet.as_view(), target, player) for target in targets
                )
                total += max(0.0, roi)
            threat = incoming_threat(
                planet.x, planet.y, player, fleet_views, horizon_turns=20
            )
...
def _state_signature(state: SearchState) -> tuple:
    planets_sig = tuple(
        (planet.id, planet.owner, planet.ships) for planet in state.planets
    )
    fleets_sig = tuple(
        sorted(
            (
...
def _rollout_phase1_baseline(
    ...
    for _ in range(max(0, depth - 1)):
        state = _apply_actions(
            state, _phase1_decisions(state, player), root_depth=False
        )
        state = _advance_one_turn(_simulate_opponents(state, player))
...
def search(obs: Any, player: int, time_budget_sec: float = 0.3) -> list[list[float]]:
    """Return moves chosen by lightweight beam search."""
    parsed_player, raw_planets, raw_fleets, step = _parse_obs(obs)
    if player != parsed_player:
        player = parsed_player

    # v0 簡略化: raw obs は fleet の target_planet_id を公開しないため -1 に固定し、
    # turns_remaining=99 で depth<=2 の探索範囲内には到達しない扱いにする。
    # 結果として「敵 fleet が空中に留まり孤島化する」挙動になる（将来改善ポイント）。
    initial = SearchState(
