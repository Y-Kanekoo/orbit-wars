...
def angle_to(ax: float, ay: float, bx: float, by: float) -> float:
    """a→b の角度（radians、atan2）。"""
    return math.atan2(by - ay, bx - ax)


def segment_hits_sun(
    x0: float, y0: float, x1: float, y1: float, margin: float = 0.5
) -> bool:
    """線分 (x0,y0)→(x1,y1) が太陽（円）と交差するか。

    margin: 太陽半径への余裕。大きいほど安全側に倒す。
    """
    cx, cy = SUN_CENTER
    r = SUN_RADIUS + margin
    dx, dy = x1 - x0, y1 - y0
...
def predict_planet_position(
    ...
    """公転惑星の turns_ahead ターン後の位置を予測する。

    cx, cy: 回転中心（通常 SUN_CENTER）
    initial_x, initial_y: ゲーム開始時の位置
    angular_velocity: radians/turn
    """
    dx = initial_x - cx
    dy = initial_y - cy
...
def avoidance_angle(
    ...
    """太陽に当たらない角度を返す（直線がヒットする場合は回避方向にずらす）。"""
