...
# H016 cycle 2 (2026-06-04): NN value head を beam leaf eval に env-gated 加算。
# advisor 助言: MCTS+NN は cycle 1 で gate FAIL (sniper 0.4667/prev_best 0.3333) だが、
# NN value 自体は MCTS strong-winrate を 2× 改善 (exp 028 比 sniper 0.20→0.47, prev 0.17→0.33)。
# NN を beam に組込み distribution skew (train=noop beam state, query=MCTS state) を解消し
# 真の signal を測る診断。ORBIT_WARS_NN_VALUE_BEAM=1 + ORBIT_WARS_NN_VALUE_MODEL=<path> で有効、
# 既定 OFF で agent byte 不変 (LB 435.3 維持)。
NN_VALUE_BEAM_WEIGHT = float(os.environ.get("ORBIT_WARS_NN_VALUE_BEAM_WEIGHT", "0.0"))
NN_VALUE_BEAM_SCALE = (
    100.0  # NN value (-1, 1) を _score_state スケールに揃える (TERRITORY_SCALE と同型)
)
_NN_VALUE_MODEL_CACHE: Any = None  # None=未試行 / False=load 失敗 or disabled / model=ready


def _get_nn_value_model_for_beam() -> Any:
    """ORBIT_WARS_NN_VALUE_BEAM=1 で初回呼出時に lazy load (failure 時は None で safe fallback)。"""
    global _NN_VALUE_MODEL_CACHE
    if _NN_VALUE_MODEL_CACHE is not None:
        return _NN_VALUE_MODEL_CACHE if _NN_VALUE_MODEL_CACHE is not False else None
    if os.environ.get("ORBIT_WARS_NN_VALUE_BEAM", "0") != "1":
        _NN_VALUE_MODEL_CACHE = False
        return None
    path = os.environ.get("ORBIT_WARS_NN_VALUE_MODEL", "")
    if not path or not os.path.exists(path):
        _NN_VALUE_MODEL_CACHE = False
        return None
    try:
        from src.nn.value_infer import ValueModel  # noqa: PLC0415

        _NN_VALUE_MODEL_CACHE = ValueModel(path)
        return _NN_VALUE_MODEL_CACHE
    except Exception:  # noqa: BLE001
        _NN_VALUE_MODEL_CACHE = False
        return None


def _nn_value_term(state: SearchState, player: int) -> float:
    """NN value を beam の eval に加算する項 (model 未 load なら 0.0 で safe)。"""
    model = _get_nn_value_model_for_beam()
    if model is None:
        return 0.0
    try:
        from src.nn.value_infer import encode_search_state  # noqa: PLC0415

        encoded = encode_search_state(state, player)
        return float(model.evaluate(encoded))
    except Exception:  # noqa: BLE001
        return 0.0
...
def _score_state(state: SearchState, player: int) -> float:
    ...
    # H016 cycle 2: NN value 加算 (ORBIT_WARS_NN_VALUE_BEAM_WEIGHT > 0 で有効、既定 0.0 で inert)
    if NN_VALUE_BEAM_WEIGHT > 0.0:
        total += NN_VALUE_BEAM_WEIGHT * NN_VALUE_BEAM_SCALE * _nn_value_term(state, player)
    return total
