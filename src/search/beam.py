...
# H009 multi-launch: 1 planet が 1 turn に最大 K 回 launch できるよう per-planet パスを K 回繰り返す。
# K=1 で baseline と完全 parity (ループ 1 回 = 現状)。各パスは残 ships から再評価するため over-commit しない。
MULTI_LAUNCH_K = 3
...
def _expand_turn(
    ...
    # H009: per-planet パスを K 回。各パスで planet は残 ships から追加 launch しうる。
    # K=1 なら従来の単一 launch と完全 parity。wait 候補が常に含まれるため追加 launch 不要な
    # 状態は dedup で維持される。
    for _ in range(MULTI_LAUNCH_K):
