...
# H024: raw obs の in-flight fleet は target を持たないため angle から target を推定する。
# bearing のズレがこの許容角を超える fleet は frozen (探索内未着弾) のまま残す。
TARGET_ANGLE_TOLERANCE_RAD = math.radians(30.0)
...
def _estimate_fleet_target(
    ...
    """H024: raw obs の in-flight fleet の target planet と残り到達 turn を推定する。

    raw obs は fleet の target を持たないため、進行 angle に最も合致する非自軍 planet
    (enemy ∪ neutral) を target とみなし、ETA を 距離 / 速度 で算出する。bearing の
    ズレが許容角 (TARGET_ANGLE_TOLERANCE_RAD) を超える場合は frozen (-1, 99) を返し、
    探索内で着弾させない (誤った target 推定を避ける安全側 fallback)。
    """
    best_id = -1
    best_delta = TARGET_ANGLE_TOLERANCE_RAD
    best_distance = 0.0
    for planet in planets:
        if planet.owner == fleet_owner:
            continue
        bearing = angle_to(fx, fy, planet.x, planet.y)
        delta = abs(_normalize_angle(bearing - angle))
        if delta >= best_delta:
            continue
        best_delta = delta
        best_id = planet.id
        best_distance = distance(fx, fy, planet.x, planet.y)
    if best_id == -1:
        return -1, 99
    turns_remaining = max(1, math.ceil(best_distance / max(fleet_speed(ships), 1e-6)))
    return best_id, turns_remaining
...
def search(obs: Any, player: int, time_budget_sec: float = 0.3) -> list[list[float]]:
    ...
    initial_planets = [ProjectedPlanet.from_raw(planet) for planet in raw_planets]
    initial_fleets: list[ProjectedFleet] = []
    for fleet in raw_fleets:
        ...
        angle = float(fleet[4])
        ships = int(fleet[6])
        target_id, turns_remaining = _estimate_fleet_target(
            initial_planets, owner, fx, fy, angle, ships
        )
        initial_fleets.append(
            ProjectedFleet(
                owner=owner,
                x=fx,
                y=fy,
