...
# H020 (exp/040) endgame all-in: 生存敵 owner が 1 人のみ かつ 自軍 ship 数が
# 敵 ship 数 × ENDGAME_DOMINANCE_RATIO を超える endgame で、全 own planet が
# 最寄り敵 planet へ全 ship を集中派兵する。bounded late-game heuristic で
# mid-game mirror match (no-regression gate の主信号) に介入しない。
ENDGAME_DOMINANCE_RATIO = 1.5
...
def _ship_totals(state: SearchState, player: int) -> tuple[int, int, set[int]]:
    """(自軍 ship 総数, 敵 ship 総数, 生存中の敵 owner 集合) を返す。
    planet garrison + in-flight fleet ships を合算 (owner==-1 の neutral は除外)。"""
    own = 0
    enemy = 0
    ...
    for planet in state.planets:
        if planet.owner == player:
            ...
            enemy += planet.ships
            enemy_owners.add(planet.owner)
    for fleet in state.fleets:
        if fleet.owner == player:
            own += fleet.ships
        elif fleet.owner >= 0:
            enemy += fleet.ships
            enemy_owners.add(fleet.owner)
    return own, enemy, enemy_owners


def _endgame_all_in_moves(state: SearchState, player: int) -> list[list[float]] | None:
    """H020: 生存敵 owner が 1 人 かつ 自軍 ship 数 > 敵 × ENDGAME_DOMINANCE_RATIO の
    endgame で、全 own planet が最寄り敵 planet へ全 ship を集中派兵する moves を返す。
    条件を満たさなければ None (通常の beam search に委譲)。"""
    own_ships, enemy_ships, enemy_owners = _ship_totals(state, player)
    if len(enemy_owners) != 1:
        return None
    own_planets = [planet for planet in state.planets if planet.owner == player]
    enemy_planets = [
        planet for planet in state.planets if planet.owner >= 0 and planet.owner != player
    ]
    if not own_planets or not enemy_planets:
        return None
    if own_ships <= enemy_ships * ENDGAME_DOMINANCE_RATIO:
        return None

    moves: list[list[float]] = []
    for source in sorted(own_planets, key=lambda planet: planet.id):
        if source.ships <= 0:
            continue
        target = min(
            enemy_planets,
            key=lambda t: (distance(source.x, source.y, t.x, t.y), t.id),
        )
        angle = float(avoidance_angle(source.x, source.y, target.x, target.y, margin=1.0))
        moves.append([int(source.id), angle, int(source.ships)])
...
def search(obs: Any, player: int, time_budget_sec: float = 0.3) -> list[list[float]]:
    ...
    # H020: endgame all-in override (dominance 条件を満たす時のみ beam を bypass)
    endgame_moves = _endgame_all_in_moves(initial, player)
    if endgame_moves is not None:
        return endgame_moves
