...
| H024 | discarded | 1 | 5 | 4 | +60 | beam forward sim の **fleet projection 修正** (beam.py L488-490): 敵 fleet を target=-1/turns_remaining=99 固定でなく、最近接 enemy planet を target 推定し ETA で探索内到達させる。**最小実装** (target 推定のみ、他 knob は baseline 据置)。fleet projection freeze が helps-weak/hurts-strong (4 例) の dominant downstream 真因 | **exp/024 で discard**: mix-eval random 0.9333↑ / nearest_sniper 0.6↑ / **prev_best 0.4667↓** (baseline 0.5667, winrate_min 0.4667<0.50 gate FAIL)。H024 自身の parity 基準 (prev_best shift 下落で targeting wrong) に抵触。決定的反証: random/sniper/prev_best が H023 と wins カウント完全一致 (28/18/14) → fleet projection freeze は dominant 真因でない (advisor 診断を反証、5 系統目の helps-weak/hurts-strong)。beam.py は baseline (fleet freeze, depth=2) に revert。次手は個別 knob 打切り → H011 MCTS パラダイム置換 or prev_best mix 化で測定特性切分け | advisor 助言。learned_rules INFO `fleet_projection_freeze_not_root_cause` 昇格 |
