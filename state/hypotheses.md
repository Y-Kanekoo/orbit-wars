...
| H021 | active | 1 | 5 | 4 | +0 (基盤) | mix-eval infra: tournament.py に `--opponents random,nearest_sniper,prev_best` 追加 (N=30/相手)。best_score.json schema v2 で mix_eval section。kaggle-quota-guard hook に submit gate enforce。**全 hypothesis の prerequisite** | submit gate = winrate_random≥0.90 AND winrate_sniper≥0.60 AND winrate_prev_best≥(prev+0.05) AND winrate_min≥0.50 AND errors_total=0 | exp 002 教訓 (単一相手 winrate は LB を予測しない、LB regression -17.3); plan wise-wibbling-russell.md |
...
| H002 | blocked | 1 | 5 | 4 | +100 | 30 turn 先までの ship 在庫 projection (production + incoming fleet 減算) を eval に加算 (係数 0.3-0.8 sweep)。projection.py 実装済・eval 未統合 | **H021 (mix-eval) 完了後 active 化**。H021 gate で評価 (winrate_min≥0.50) | PLAN.md L268; H021 依存 |
