...
| H021 | done | 1 | 5 | 4 | +0 (基盤) | mix-eval infra: tournament.py に `--opponents random,nearest_sniper,prev_best` 追加 (N=30/相手, seat split)。best_score.json schema v2 で mix_eval section。kaggle-quota-guard hook + submit.sh に submit gate enforce。**全 hypothesis の prerequisite** | submit gate = winrate_random≥0.90 AND winrate_sniper≥0.60 AND winrate_prev_best≥(prev+0.05) AND winrate_min≥0.50 AND errors_total=0 | exp/021 完了 (PR 提出)。baseline mix_eval: random 0.8667 / sniper 0.5667 / prev_best 0.5667。注意: baseline は random 0.8667 で gate 0.90 を下回るため次 hypothesis は random も改善要 |
...
| H002 | active | 1 | 5 | 4 | +100 | 30 turn 先までの ship 在庫 projection (production + incoming fleet 減算) を eval に加算 (係数 0.3-0.8 sweep)。projection.py 実装済・eval 未統合 | H021 mix-eval gate で評価 (random≥0.90, sniper≥0.60, prev_best≥0.6167, winrate_min≥0.50, errors=0)。H021 完了で unblock | PLAN.md L268; projection.py (PR #36) 実装済、eval 統合が残作業 |
