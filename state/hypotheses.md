...
| H004 | in_progress | 1 | 5 | 6 | +60 | beam tuning (search 改善, eval 不変): **実証 (exp/004): budget はボトルネックでない** — depth=2/width=16 で beam は max 6ms/budget 300ms しか使わず、width sweep (16/24/32) は frontier 飽和で timing 完全同一 (max 18ms 不変) = **no-op**、budget bump は depth=3 でも max 27ms<<700ms。よって H004a/b/c (budget/width のみ) は moves 不変が確定 → skip。唯一挙動を変えるレバーは **depth 2→3** (H004d 相当, max 27ms で timing 安全)。本 iter は depth=3 (width=16 据置, budget 0.3→0.7 は headroom only) を実施 | timing pre-gate (test_timing 950ms) + mix-eval no-regression gate + max_act_ms<800、prev_best≥0.6167 で submit | plan wise-wibbling-russell.md「次のタスク」。a/b/c は実証で no-op 判明し skip (frontier 飽和 + 予算過剰) |
