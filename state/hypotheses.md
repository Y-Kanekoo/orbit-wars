...
| H009 | awaiting_lb | 1 | 4 | 5 | +60 | multi-launch per planet: 1 turn の compound action を最大 K=3-5 launch list で生成 | vs H004 N=30 winrate≥55% | PLAN.md B-3。**exp/032 submitted (2026-05-30, sub 53173411)**: `_expand_turn` の per-planet パスを `MULTI_LAUNCH_K=3` 回繰り返す最小実装 (K=1 で baseline 完全 parity)。mix-eval 2 回測定とも no-regression gate PASS (random 1.0/0.8667, sniper 0.6, **prev_best 0.5667/0.6 で LB 予測軸 非 regression** = 史上初 regression-free 改善)。ただし prev_best gain は flat~+1試合 で noise レベル。**STATUS awaiting_lb**: PR は merge せず supervisor が 24-48h 安定 LB で判定 |
