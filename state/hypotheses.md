...
| H005 | in_progress | 1 | 4 | 4 | +50 | sun blocking aware path planning: avoidance_angle の単一 12° stub を robust 化 (偏向後経路が sun を抜けるまで段階的に増やす)。fleet が straight-line 移動かつ sun-crossing で破壊される env 仕様に対し、現 stub は 12° で抜けない near-tangent 経路で fleet を sun に捨てる。出力 move の角度のみ変更 (beam 内部 sim 不変) で bounded downside | mix-eval no-regression gate (random≥0.8667 / sniper≥0.5667 / prev_best≥0.6167 / winrate_min≥0.50 / errors=0) | PLAN.md L271, B-12。avoidance 改善は探索/eval 非関与のため helps-weak/hurts-strong 7系統と別カテゴリ (advisor 助言) |
