...
| H026 | in_progress | 2 | 5 | 2 | +0 (診断) | MCTS rollout policy を `_phase1_decisions` → uniform random に置換した A/B 診断 (mcts.py `_rollout_value`)。exp028 の MCTS strong-opponent 激減 (sniper 0.20 / legacy 0.1667) が phase1 rollout policy 由来かを切り分ける。`ROLLOUT_POLICY=uniform` env で切替、main default (beam) parity 不変 | relaxed gate (random>=0.8667 AND sniper>=0.5667 AND winrate_min(random,sniper)>=0.50 AND errors=0) を `ORBIT_WARS_MCTS=1 ROLLOUT_POLICY=uniform` で測定し phase1 版 (exp028) と比較。MCTS は env OFF (提出物外) のため **no-submit 診断** (LB/quota 影響ゼロ) | learned_rules L61 推奨 (b); advisor 助言。mcts.py のみ改変。H011/exp028 lesson の真因切り分け (a)(b)(c) のうち (b) を消し込む |
