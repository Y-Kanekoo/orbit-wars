...
| H024 | in_progress | 1 | 5 | 4 | +60 | beam forward sim の **fleet projection 修正** (beam.py L488-490): 敵 fleet を target=-1/turns_remaining=99 固定でなく、最近接 enemy planet を target 推定し ETA で探索内到達させる。**最小実装** (target 推定のみ、他 knob は baseline 据置)。fleet projection freeze が helps-weak/hurts-strong (4 例) の dominant downstream 真因 | mix-eval no-regression gate + **parity** (fleet targeting 追加で prev_best mean が baseline 0.5667 未満に shift しないこと、shift したら targeting heuristic が wrong)。**gate pass で即 submit** (4 月以降 LB 改善ゼロ) | advisor 助言。H023 診断で opponent model 主因否定 → fleet projection freeze 特定。H011 MCTS/depth/eval より優先 (正しい dynamics の上でこそ機能)。H024 pass 後に depth/eval/H023 を re-explore |
