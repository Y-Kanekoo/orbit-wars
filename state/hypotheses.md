...
| H004 | active | 1 | 5 | 6 | +60 | beam tuning (search 改善, eval 不変): budget bump (main.py `_BEAM_TIME_BUDGET_SEC` 0.3→0.7、deadline 0.90s に対し ~0.6s 未使用を解放) を第一軸に、(depth,width,budget) を H004a(2,16,0.7)→b(2,24,0.7)→c(2,32,0.7)→d(3,16,0.7)→e(3,24,0.7) で 1 iter 1 候補 sweep。eval 不変で strong opponent regression 回避 | timing pre-gate (test_timing 800ms) + mix-eval no-regression gate + max_act_ms<800、prev_best≥0.6167 で submit | plan wise-wibbling-russell.md「次のタスク」。budget がボトルネック (depth/width のみ上げても time-guard で no-op sweep) |
