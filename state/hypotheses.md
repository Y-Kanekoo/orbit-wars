...
| H009 | in_progress | 1 | 4 | 5 | +60 | multi-launch per planet: 1 turn の compound action を最大 K=3-5 launch list で生成 | vs H004 N=30 winrate≥55% | PLAN.md B-3。**exp/032 着手 (2026-05-30)**: `_expand_turn` の per-planet パスを `MULTI_LAUNCH_K=3` 回繰り返す最小実装 (K=1 で baseline 完全 parity)。action-space breadth は 7 系統未試の唯一機構 (advisor)。gate は check_mix_gate.py そのまま (prev_best≥0.6167)、緩めない |
